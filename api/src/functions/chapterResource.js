...
async function chapterResourceHandler(request, context) {
  try {
    ...
    const remoteUrl = getChapterResource(chapterId, content);
    ...
    const errorMessage = `Error generating PDF URL: error.message`;
...
app.http('chapterResource', {
  ...
  handler: chapterResourceHandler,
});

module.exports = { chapterResourceHandler };
